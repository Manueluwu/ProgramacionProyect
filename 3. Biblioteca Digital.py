...
class MaterialDigital(Material):
    def __init__(self, titulo, archivo, enlace, estado="disponible"):
        super().__init__(titulo, estado)  # Se corrige para pasar 'estado'
...
class Persona:
    ...
    def __init__(self, nombre, correo, edad, rol):
        ...
        self.edad = edad
        self.rol = rol
        Persona.lista.append(self)  # Se registra la persona automáticamente al crearse

    def registrar(self):
        # Ya se agregó en __init__, solo se muestra el mensaje.
...
class Usuario(Persona):
    def __init__(self, nombre, correo, edad):
        ...
        self.materiales_prestados = []  # Se usa minúscula para consistencia
        self.penalizaciones = []  
...
class Bibliotecario(Persona):
    ...
    def gestionar_prestamo(self, usuario, material, sucursal):
        if material in sucursal.catalogo and material.estado.lower() == "disponible":
...
# -----------------------
# Clases de Gestión de Catálogo y Sucursales
# -----------------------
...
class Sucursal:
    ...
    def buscar_materiales_disponibles(self):
        return [material for material in self.catalogo if material.estado.lower() == "disponible"]
    ...
    def buscar_materiales(self, criterio, valor):
        ...
        for sucursal in self.sucursales:
            for material in sucursal.catalogo:
                # Búsqueda según el tipo de material y el criterio
                if isinstance(material, Libro):
                    ...
                    if criterio == "archivo" and material.archivo == valor:
                        resultados.append(material)
                    elif criterio == "titulo" and material.titulo == valor:
                        resultados.append(material)
        return resultados
...
# -----------------------
# Clases de Préstamos y Penalizaciones
# -----------------------
...
if __name__ == "__main__":
    ...
    # Registrar personas (solo se muestra mensaje, ya se registraron en __init__)
    usuario1.registrar()
